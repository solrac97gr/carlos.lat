...
<CodeSnipet language={"bash"} code={`go test -coverprofile=coverage.out ./… && go tool cover -html=coverage.out && rm coverage.out`} />
...
<CodeSnipet language={"bash"} code={`go test -cpuprofile=cpu.out ./… && go tool pprof cpu.out && rm cpu.out`} />
